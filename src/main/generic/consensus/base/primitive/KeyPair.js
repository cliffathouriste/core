class KeyPair extends Primitive {
    ...
    static async deriveDeepLocked(buf, key) {
        ...
        const salt = buf.read(32);
        const check = buf.read(4);
    ...
    async deepLock(key) {
        ...
        const buf = new SerialBuffer(this.privateKey.serializedSize + 32 + 4);
        buf.write(await KeyPair._otpKdf(this.privateKey.serialize(), key, salt, KeyPair.DEEP_LOCK_ROUNDS));
        buf.write(salt);
        buf.write((await this.publicKey.hash()).subarray(0, 4));
