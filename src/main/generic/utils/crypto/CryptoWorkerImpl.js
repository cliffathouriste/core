class CryptoWorkerImpl extends IWorker.Stub(CryptoWorker) {
    ...
    async kdf(key, salt, iterations) {
        ...
        try {
            ...
            const wasmSeed = Module.stackAlloc(salt.length);
            new Uint8Array(Module.HEAPU8.buffer, wasmSeed, salt.length).set(salt);
            const res = Module._nimiq_kdf(wasmOut, wasmIn, key.length, wasmSeed, salt.length, 512, iterations);
            if (res !== 0) {
                throw res;
            }
            const hash = new Uint8Array(CryptoWorker.HASH_SIZE);
